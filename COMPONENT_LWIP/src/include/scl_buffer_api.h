...
/** Sets the current size of a SCL packet
 *
 *  Implemented in the port layer interface which is specific to the
 *  buffering scheme in use.
 *  This function sets the current length of a SCL packet buffer
 *
 ...
 *  @return       : SCL_SUCCESS or error code
 */
scl_result_t scl_buffer_set_size(scl_buffer_t buffer, uint16_t size);
 ...
 *  this function allows SCL to move  the current 'front' location pointer so that it has space to
 *  add headers to transmit packets, and so that the network stack does not see the internal SCL
 ...
 *  @return                  : SCL_SUCCESS or error code
 */
scl_result_t scl_buffer_add_remove_at_front(scl_buffer_t *buffer, int32_t add_remove_amount);
